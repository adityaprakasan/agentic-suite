...
def calculate_token_cost(prompt_tokens: int, completion_tokens: int, model: str) -> Decimal:
    try:
        ...
        # Detailed logging if model or pricing is missing
        if not model_obj:
            logger.error(f"[COST_CALC] ❌ Model object is None for '{resolved_model}' (original: '{model}')")
            logger.error(f"[COST_CALC] Available models: {list(model_manager.registry._models.keys())[:10]}...")
            return Decimal('0.01')
        
        if not model_obj.pricing:
            logger.error(f"[COST_CALC] ❌ Pricing is None for model '{resolved_model}'")
            logger.error(f"[COST_CALC] Model object: {model_obj}")
            return Decimal('0.01')
        ...
        logger.error(f"[COST_CALC] ❌ Exception calculating token cost for model '{model}': {e}", exc_info=True)
        return Decimal('0.01')
