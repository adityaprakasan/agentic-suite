...
'''
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "presentation_name": {
                        "type": "string",
                        "description": "Name of the presentation to present"
                    }
                },
                "required": ["presentation_name"]
            }
    ...
    async def present_presentation(self, presentation_name: str) -> ToolResult:
        """Present the final presentation to the user with all slides and metadata"""
        try:
            await self._ensure_sandbox()
            ...
            presentation_path = f"{self.workspace_path}/{self.presentations_dir}/{presentation_name}"
            ...
            # Get all slide files
            try:
                files = await self.sandbox.fs.list_files(presentation_path)
                slide_files = sorted([f for f in files if f.name.endswith('.html')], 
                                   key=lambda x: x.name)
                
                # Prepare file attachments for the response
                attachments = []
                for slide_file in slide_files:
                    slide_path = f"{presentation_path}/{slide_file.name}"
                    attachments.append({
                        "name": slide_file.name,
                        "path": f"/workspace/{self.presentations_dir}/{presentation_name}/{slide_file.name}",
                        "type": "slide"
                    })
                
                return self.success_response({
                    "message": f"Presentation '{presentation_name}' is ready!",
                    "presentation_name": presentation_name,
                    "title": metadata.get("title", presentation_name),
                    "description": metadata.get("description", ""),
                    "total_slides": len(slide_files),
                    "slides": [s.name for s in slide_files],
                    "attachments": attachments,
                    "presentation_path": f"/workspace/{self.presentations_dir}/{presentation_name}",
                    "actions_available": [
                        "Download as PDF (use export_to_pdf)",
                        "Download as PPTX (use export_to_pptx)",
                        "Upload to Google Slides"
                    ]
                })
                
            except Exception as e:
                return self.fail_response(f"Failed to load presentation files: {str(e)}")
                
        except Exception as e:
            return self.fail_response(f"Failed to present presentation: {str(e)}")
